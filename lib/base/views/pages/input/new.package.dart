        ...
        child: ListView(
          ...
          children: [
                                ...
                                .map(
                                  (i) => Container(
                                    ...
                                    child: InkWell(
                                      ...
                                      onTap: () => setState(() => packageImage
                                          .removeAt(packageImage.indexOf(i))),
                                      child: Container(
            ...
            inputField(
                ...
                "Description",
                (val) => con.package.description = val,
                "description",
                (val) => con.setError(val)),
            inputField(
                context,
                ...
                "Approximate value",
                (val) => con.package.approximateValue = val,
                "number",
                (val) => con.setError(val)),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                ...
                Row(
                  children: <Widget>[
                    radioItem("Standard (0-5Kg)", weight, "Standard (0-5Kg)",
                        (val) => setState(() => weight = val)),
                    radioItem("Parcel (5Kg Plus)", weight, "Parcel (5Kg Plus)",
                        (val) => setState(() => weight = val)),
                  ],
                ),
              ],
            ...
            pickAddressFields(context, "Shipment Address", "Address", () {
              con.pickLocation().then((value) {
                con.package.shipmentAddress = PackageAddress(address: value);
                setState(() {});
              });
            }, address: con.package.shipmentAddress),
            inputField(
                context,
                "Shipment nearest intersection",
                "Intersection",
                (val) => con.package.shipmentAddress!.intersection = val,
                "text",
                (val) => con.setError(val)),
            mainHeading(context, "Destination Address"),
            pickAddressFields(context, "Destination Address", "Address", () {
              con.pickLocation().then((value) {
                con.package.destinationAddress = PackageAddress(address: value);
                setState(() {});
              });
            }, address: con.package.destinationAddress),
            inputField(
                context,
                "Destination nearest intersection",
                "Intersection",
                (val) => con.package.destinationAddress!.intersection = val,
                "text",
                (val) => con.setError(val)),
